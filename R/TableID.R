...
##' @export
totalMatrix<-function()# The totalMatrix function will generate a NA 30*15 matrix for the game.
{
  tablebg<-matrix(0,nrow = 30,ncol = 15)  #Generate a NA 30*15 matrix for the game
  rownames(tablebg)<-rev(as.character(1:30))#Name rownames as 1:30 numbers of characters, using the react function to return an inverse version of the data
  colnames(tablebg)<-as.character(1:30)#Name colnames as 1:30 numbers of characters, using the react function to return an inverse version of the data
  tablebg
}
